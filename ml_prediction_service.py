...
print(f"[{rank}]: MPI process initialized. Rank: {rank}, Total Size: {size}") # Added print
...
try:
    ...
    # Get the default number of worker processors, default to 5 if not found in config
    DEFAULT_WORKER_PROCESSORS = getattr(config, 'DEFAULT_WORKER_PROCESSORS', 5)
    print(f"[{rank}]: Configuration loaded successfully.") # Added print
except ImportError:
...
def pull_from_queue(queue_url, token):
    print(f"[{comm.rank}]: Attempting to pull from queue: {queue_url}/pull") # Added print
    headers = {'Authorization': token}
    try:
        # Append /pull to the base URL
        response = requests.get(f"{queue_url}/pull", headers=headers)
        response.raise_for_status()
        print(f"[{comm.rank}]: Successfully pulled from queue {queue_url}/pull. Status: {response.status_code}") # Added print
        return response.json()
    except requests.exceptions.HTTPError as e:
        if response.status_code == 400 and "Queue is empty" in response.text:
            print(f"[{comm.rank}]: Queue {queue_url}/pull is empty (Status 400).") # Added print
            return None
...
def push_to_queue(queue_url, data, token):
    print(f"[{comm.rank}]: Attempting to push to queue: {queue_url}/push with data (transaction_id): {data.get('transaction_id', 'N/A')}") # Added print
    headers = {'Authorization': token, 'Content-Type': 'application/json'}
    try:
        # Append /push to the base URL
        response = requests.post(f"{queue_url}/push", headers=headers, json=data)
        response.raise_for_status()
        print(f"[{comm.rank}]: Successfully pushed to queue {queue_url}/push. Status: {response.status_code}") # Added print
        return response.json()
...
def load_ml_model(filename):
    print(f"[{comm.rank}]: Loading ML model from {filename}...") # Added print
    if not os.path.exists(filename):
        raise FileNotFoundError(f"Model file {filename} not found.")
    model = joblib.load(filename)
    print(f"[{comm.rank}]: Model loaded successfully.") # Added print
    return model
...
def preprocess_single_transaction(transaction_data, statuses_map):
    ...
    """
    print(f"[{comm.rank}]: Preprocessing transaction ID: {transaction_data.get('transaction_id', 'N/A')}") # Added print
    ...
    try:
        ...
        print(f"[{comm.rank}]: Timestamp converted for {transaction_data.get('transaction_id', 'N/A')}") # Added print
    except Exception as e:
    ...
    print(f"[{comm.rank}]: Status mapped for {transaction_data.get('transaction_id', 'N/A')}") # Added print
    ...
    print(f"[{comm.rank}]: Preprocessing complete for {transaction_data.get('transaction_id', 'N/A')}") # Added print
    return X
...
if rank == 0:
    ...
    try:
        ...
        for i in range(1, size):
            print(f"Master: Sending SHUTDOWN signal to worker {i} due to missing model.") # Added print
            comm.send("SHUTDOWN", dest=i, tag=99) # Using a shutdown tag
        exit(1)
    except Exception as e:
        print(f"Master: Error loading model: {e}")
        for i in range(1, size):
            print(f"Master: Sending SHUTDOWN signal to worker {i} due to model load error.") # Added print
    ...
    for queue_name in ['transactions', 'results']:
        ...
        print(f"Master: Attempting to create/confirm queue: {queue_name} at {queue_full_url}") # Added print
        try:
            ...
            for i in range(1, size):
                print(f"Master: Sending SHUTDOWN signal to worker {i} due to queue service connection error.") # Added print
                comm.send("SHUTDOWN", dest=i, tag=99)
            exit(1)
        except Exception as e:
            print(f"Master: An unexpected error occurred during '{queue_name}' queue setup: {e}")
    print("Master: All necessary queues checked/created.") # Added print

    # Main loop for managing tasks
    while True:
        print(f"\nMaster: Entering new batch processing cycle. Attempting to pull up to {num_workers} requests...") # Added print
        ...
        for _ in range(num_workers):
            ...
            if request_data:
                ...
                print(f"Master: Successfully pulled transaction {request_data.get('transaction_id', 'N/A')} from transactions queue.") # Added print
            else:
                print(f"Master: No more requests found in queue or error on pull attempt. Pulled {len(current_batch_requests)} in this cycle.") # Added print
        ...
        for i in range(len(current_batch_requests)):
            ...
            print(f"Master: Sending task {task_data.get('transaction_id', 'N/A')} to worker {i+1} (tag 11).") # Added print
            comm.send(task_data, dest=i + 1, tag=11)
            print(f"Master: Sent task {task_data.get('transaction_id', 'N/A')} to worker {i+1} successfully.") # Added print
        ...
        for _ in range(len(current_batch_requests)):
            ...
            print("Master: Waiting to receive a result from any worker (tag 22)...") # Added print
            prediction_result = comm.recv(source=MPI.ANY_SOURCE, tag=22)
            pending_results.append(prediction_result)
            print(f"Master: Received result from worker {prediction_result.get('worker_rank')} for transaction {prediction_result.get('transaction_id', 'N/A')}.") # Added print
        ...
        for res in pending_results:
            ...
            print(f"Master: Pushed prediction for transaction {res.get('transaction_id', 'N/A')} to results queue.") # Added print
...
# Worker Process Logic (rank != 0)
    ...
    while True:
        print(f"Worker {rank}: Waiting for task from master (tag 11 or SHUTDOWN tag 99)...") # Added print
        try:
            ...
            print(f"Worker {rank}: Received message from master with tag {status.Get_tag()}.") # Added print
            ...
            if not isinstance(task_data, dict): # Basic check if received data is actually a dictionary
                print(f"Worker {rank}: Received invalid task data (not a dict): {task_data}. Skipping.") # Added detail
                continue

            transaction_id = task_data.get('transaction_id', 'N/A_NO_ID')
            print(f"Worker {rank}: Received task for transaction {transaction_id}.") # Detailed print
            ...
            try:
                ...
                print(f"Worker {rank}: Prediction for {transaction_id} is: {prediction_result['prediction']}") # Added print
            ...
            print(f"Worker {rank}: Sending result for transaction {transaction_id} back to master (tag 22).") # Detailed print
            comm.send(prediction_result, dest=0, tag=22)
            print(f"Worker {rank}: Sent result for transaction {transaction_id} successfully.") # Added print
